...
typedef struct RGB
{
	unsigned char red;	//0.0 ~ 1.0
	unsigned char green;	//0.0 ~ 1.0
	unsigned char blue;	//0.0 ~ 1.0
}RGB;//±¸Á¶Ã¼
...
const bool writeBMP24(const char* filename, const int x_res, const int y_res, RGB* rgb_array)
{
	int i, j, ipos;
	int bytesPerLine;
...
//      data_.initialize(0, 0, width, height, false);
	...
	*rgb_array = (RGB*)malloc(sizeof(RGB) * (width * height * 3));
